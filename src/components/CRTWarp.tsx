/*
 * Adapted from React Bits CRTWarp by David Haz.
...
export function CRTWarp({
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uTime: { value: 0 },
        uSpeed: { value: 0.5 },
        uColor: { value: new THREE.Color('#7cab3d') },
        uBackgroundColor: { value: new THREE.Color('#060906') },
        uCurvature: { value: 0.3 },
        uScanlineStrength: { value: 0.34 },
        uScanlineFrequency: { value: 220 },
    ...
    try {
      ...
      console.warn('CRTWarp unavailable; using CSS fallback.', error);
  ...
  }, [backgroundColor, bloom, bloomRadius, brightness, color, curvature, mouseReact, mouseStrength, noise, pixelation, rgbShift, scanlineFrequency, scanlineStrength, speed, vignette, waveAmplitude, waveFrequency]);

  return <div ref={containerRef} className={`crt-warp-container ${className ?? ''}`.trim()} style={style} aria-hidden="true" />;
}
