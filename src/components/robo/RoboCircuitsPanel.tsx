      ...
      <div className="robo-circuit-list">
        {circuits.map((circuit) => (
          <article key={circuit.id} className={`robo-circuit robo-circuit--${circuit.id}`}>
            <div className="robo-circuit__header"><div><strong>{circuit.name}</strong><span>{circuit.tierLabel}</span></div></div>
            <p className="robo-circuit__target">{circuit.tierProgressLabel}</p>
            ...
            <div className="robo-circuit__footer">
              ...
              {circuit.bottleneckCostLabel && <button type="button" onClick={() => onBuyBottleneck(circuit.id)} disabled={!circuit.canBuyBottleneck}><span>{guide.buyMissing}</span><strong>{circuit.bottleneckCostLabel} RG</strong></button>}
            </div>
          </article>
        ))}
      </div>
