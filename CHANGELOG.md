...
- **Goblin Blocks**, the Hoard Warehouse: an 8×8 block-placement puzzle reached from a new giver in the spawn pit, unlocked for good by your first Warren Den. Three pieces at a time, no rotation and no timer; fill a row or column and the porters haul it away. A run ends when none of the pieces left in the tray fits anywhere, and pays the warren in seconds of its own production plus Hoard Tokens, which buy warehouse charges and never production. The first run each day pays in full and later runs taper, so it stays worth playing without ever becoming a chore. Runs survive closing the game and a Great Migration alike. Playable by drag or entirely by keyboard, with painted loot-crate art, screen-reader announcements and reduced-motion fallbacks, in all seven interface languages. Design and balance notes live in `docs/goblin-blocks/`.
